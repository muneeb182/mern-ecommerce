...
const ProductTabs = ({userInfo , rating , setRating , comment, setComment ,product , loadingProductReview ,submitHandler}) => {
                  ...
                  {userInfo ? (
                    <form onSubmit={submitHandler}>
                        ...
                        <div className="my-2">
                            ...
                            className='p-2 border rounded-lg xl:w-[40rem] text-black border-gray-200'
                            >

                            </textarea>
                        </div>
                        <button type='submit' disabled={loadingProductReview} 
                        className='bg-pink-600 text-white py-2 px-4 rounded-lg' onClick={submitHandler}
                        >Submit</button>
                    </form>
                  ):(
                    <p>
            ...
            {activeTab === 2 && (
                ...
                <div>
                    {product.reviews.map((review) => (
                        <div key={review._id} className='bg-[#f9f9f9] p-4 rounded-lg xl:ml-[2rem] sm:ml-[0rem] xl:w-[50rem] sm:w-[24rem] mb-5'>
                           <div className="flex justify-between">
                            <strong className='text-[#111111]'>{review.name}</strong>
                            <p className='bg-[#f9f9f9]'>{review.createdAt.substring(0,10)}</p>
                            </div>     
